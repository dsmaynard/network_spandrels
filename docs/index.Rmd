...
1. Immigration, no traits (scenario = `notraits_immigration_search`)
2. Radiation, no traits  (scenario = `notraits_radiation_search`)
3. Immigration, no traits, uniform search (scenario = `notraits_immigration_uniform`)
4. Immigration, binary traits (scenario = `traits_binary_immigration`)
5. Immigration, continuous traits (scenario = `traits_continuous_immigration`)
6. Radiation, binary traits (scenario = `traits_binary_radiation`)
7. Radiation, continuous traits (scenario = `traits_continuous_radiation`)
...
```{r eval=F}
outcom<-assemble_community(50,scenario="traits_binary_radiation",ntraits=100,bin.prob=.3,mutation.rate=0.05)
```
...
* P (the current trait matrix, with rows as traits and columns as species. Must default to NULL in the no traits case)

Any additional scenario-specific arguments (e.g., `my_param1` and `my_param2` in the example below) can be passed to the function through the `...` argument in assemble_communities. Importantly, if the scenario includes a trait matrix P, then this custom assembly function must also initialize the P matrix for the first species. The function must then return a list of A and P, with P=NULL if no traits. The general outline of this function typically would follow something like this, though there is no reason why it cannot include more creative scenarios (e.g., complex trait evolution models):

```
 custom_assembly_scenario<-function(x,A,P,my_param1,my_param2,...){
...
```{r eval=F}
outcom<-assemble_community(50,scenario="custom_assembly_scenario",my_param1=.5, my_param2=6)
```
